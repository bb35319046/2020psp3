...
l.110~112:visitedを全て初期化

l.114:スタックを初期化

l.115:出発点をPush

l.118:while文でスタックが空になるまでという条件で繰り返す

l.119:xにスタックからポップした値を代入する

l.121~126:visitedのx番目の値が0であるならば1にして、その地点から行ける場所をスタックに入れる

---幅優先探索---

l.197:必要な変数や配列を宣言

l.199~201:visitedを全て初期化

l.203:キューを初期化

l.204:出発点をキューに入れる

l.208:while文でキューが空になるまでという条件で繰り返す

l.210:xにキューから取り出した値を代入する

l.212~219:visitedのx番目の値が０であるならば１にして、その地点からいける場所をキューに入れる
